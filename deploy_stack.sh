...
if ! [ -x "$(command -v faas-cli)" ]; then
  case "$OSTYPE" in
    darwin*|linux*)  curl -sSL https://cli.openfaas.com | sudo sh ;;
    msys*|cygwin*)   choco install faas-cli -y ;;
    *)               echo "unknown: $OSTYPE" ;;
  esac
...
curl https://raw.githubusercontent.com/openfaas/faas/master/prometheus/alertmanager.yml -L -o ./prometheus/alertmanager.yml --silent
curl https://raw.githubusercontent.com/openfaas/faas/master/prometheus/alert.rules.yml -L -o ./prometheus/alert.rules.yml --silent
curl https://raw.githubusercontent.com/openfaas/faas/master/prometheus/prometheus.yml -L -o ./prometheus/prometheus.yml --silent
